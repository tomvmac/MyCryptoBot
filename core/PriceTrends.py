...
def isDownTrend(priceTrends, strategyConfigs):
    trends = []
    ...
    for x in range(0, len(priceTrends)-1):
        ...
        if priceTrends[x + 1]["price"]:
            ...
            if percentageGainLoss <= strategyConfigs["PRICE_TRENDS_DOWNTREND_PERCENTAGE_THRESHOLD"]:
                trends.append(1)
            else:
                trends.append(0)
    ...
    for y in range(0, len(trends)):
        if currentConsecutiveDips >= strategyConfigs["PRICE_TRENDS_MIN_CONSECUTIVE_ITEMS"]:
            return True
...
def addPriceTrend(priceTrends, priceTrend, strategyConfigs):

    # Sort priceTrends
    priceTrends.sort(key=lambda x: x.get('idx'))

    if len(priceTrends) == strategyConfigs["PRICE_TRENDS_SIZE"]:
